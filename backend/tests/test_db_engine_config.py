...
def test_build_async_engine_config_pooler_uses_null_pool(monkeypatch):
    ...
    assert cfg["connect_args"] == {"prepared_statement_cache_size": 0}
    assert "pool_size" not in cfg


def test_build_async_engine_config_neon_with_pgbouncer_query_skips_connect_args(monkeypatch):
    monkeypatch.setattr(
        db,
        "settings",
        SimpleNamespace(
            database_url="postgresql+asyncpg://u:p@ep-cool-pooler.us-east-1.aws.neon.tech/neondb?pgbouncer=true",
            database_use_null_pool=False,
            database_pool_pre_ping=True,
            database_pool_recycle=1800,
            database_pool_size=10,
    ...
    assert cfg["poolclass"] is NullPool
    assert "connect_args" not in cfg
